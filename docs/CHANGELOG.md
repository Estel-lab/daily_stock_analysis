...
## [Unreleased]
- [新功能] 选股筛支持标普500全市场扫描：`SCREENER_UNIVERSE=sp500` 时对标普500成分股跑动量+价值验证并经通知层推送依据；新增 `scripts/refresh_sp500.py`（维基百科成分刷新）、`scripts/populate_sp500_fundamentals.py`（复用 yfinance 基本面适配层批量补全价值验证数据，单只失败 fail-open）、`03-sp500-fundamentals.yml`（周级刷新并提交 `data/universe/` 快照）；成分/基本面路径与 `SCREENER_UNIVERSE_LIMIT` 限速护栏可配置，缺快照时价值验证退化为仅动量，不设 `SCREENER_UNIVERSE` 时行为不变；文档见 `docs/screener.md`。
